...
#***************Control charts based on Projection depth, Algorithm "Nelder Mead" as well as the False Alarm Rate (FAR)************************#
...
load("Input_ProjectionDepthNelderMead.RData")
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size2000[[i]][["neldermead_results"]] # Take the relevant reference sample
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size3000[[i]][["neldermead_results"]]  # Take the relevant reference sample
  ranks_current_class <- c() # Container for the ranks used for the control chart
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size4000[[i]][["neldermead_results"]]   # Take the relevant reference sample
  ranks_current_class <- c() # Container for the ranks used for the control chart
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size2000[i] <- sum(ref_depths_pred <= NelderMeaddepth_results_pred_2000[i]) / length(ref_depths_pred)  # Normalized ranks 
...
CDR_testdata_pred_size2000 <- 1 - SR_testdata_pred_size2000 
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size3000[i] <- sum(ref_depths_pred <= NelderMeaddepth_results_pred_3000[i]) / length(ref_depths_pred)   # Normalized ranks 
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size4000[i] <- sum(ref_depths_pred <= NelderMeaddepth_results_pred_4000[i]) / length(ref_depths_pred)
  
}

SR_testdata_pred_size4000 <- length(which(ranks_testdata_pred_size4000 <= alpha))/length(ranks_testdata_pred_size4000)  # Normalized ranks 
CDR_testdata_pred_size4000 <- 1 - SR_testdata_pred_size4000  # Compute SR by checking how many points obtained the rank
# <= 0.05 (alpha) and normalize by the total number of points
...
PD_2_size2000 <- as.array(c(FAR_refsamples_size2000_mean, round(SR_pred_testdata_weighted_size2000, 2), round(CDR_outofcontrol_testdata_weighted_size2000, 2)))
PD_2_size3000 <- as.array(c(FAR_refsamples_size3000_mean, round(SR_pred_testdata_weighted_size3000, 2), round(CDR_outofcontrol_testdata_weighted_size3000, 2)))
PD_2_size4000 <- as.array(c(FAR_refsamples_size4000_mean, round(SR_pred_testdata_weighted_size4000, 2), round(CDR_outofcontrol_testdata_weighted_size4000, 2)))
