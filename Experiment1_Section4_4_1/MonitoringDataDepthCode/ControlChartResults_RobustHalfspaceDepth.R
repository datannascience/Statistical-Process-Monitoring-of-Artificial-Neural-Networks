...
#***************Control charts based on robust Halfspace depth as well as the False Alarm Rate (FAR) in Phase I,*******************************#
#**************Signal Rate (SR) in Phase II in-control part, and Correct Detection Rate (CDR) in Phase II,*************************************#
#****************************************************out-of-control part are computed.*********************************************************#
#**********************************************************************************************************************************************#


# Load the input depths and required labels
load("Input_RobustHalfspaceDepth.RData")
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size2000[[i]][["halfspace_robust"]] # Take the relevant reference sample
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size3000[[i]][["halfspace_robust"]]  # Take the relevant reference sample
  ranks_current_class <- c() # Container for the ranks used for the control chart
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size4000[[i]][["halfspace_robust"]]   # Take the relevant reference sample
  ranks_current_class <- c() # Container for the ranks used for the control chart
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size2000[i] <- sum(ref_depths_pred <= Testdata_results_pred_size2000[i]) / length(ref_depths_pred)
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size3000[i] <- sum(ref_depths_pred <= Testdata_results_pred_size3000[i]) / length(ref_depths_pred)   # Normalized ranks 
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size4000[i] <- sum(ref_depths_pred <= Testdata_results_pred_size4000[i]) / length(ref_depths_pred)   # Normalized ranks 
...
HD_r_size2000 <- as.array(c(FAR_refsamples_size2000_mean, round(SR_pred_testdata_weighted_size2000, 2), round(CDR_outofcontrol_testdata_weighted_size2000, 2)))
HD_r_size3000 <- as.array(c(FAR_refsamples_size3000_mean, round(SR_pred_testdata_weighted_size3000, 2), round(CDR_outofcontrol_testdata_weighted_size3000, 2)))
HD_r_size4000 <- as.array(c(FAR_refsamples_size4000_mean, round(SR_pred_testdata_weighted_size4000, 2), round(CDR_outofcontrol_testdata_weighted_size4000, 2)))
