...
#***************Control charts based on Projection depth, Algorithm "Refined Random" as well as the False Alarm Rate (FAR)*********************#
...
load("Input_ProjectionDepthRefinedRandom.RData")
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size2000[[i]][["refinedrandom_results"]] # Take the relevant reference sample
  ...
  for (k in 1:length(depths_current_class)){
    ranks_current_class[k] <- sum(depths_current_class[-k] <= depths_current_class[k]) / length(depths_current_class)  # Normalized ranks
  }
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size3000[[i]][["refinedrandom_results"]]  # Take the relevant reference sample
  ranks_current_class <- c() # Container for the ranks used for the control chart
...
for (i in 1:10){
  ...
  depths_current_class <- output_refsample_size4000[[i]][["refinedrandom_results"]]   # Take the relevant reference sample
  ranks_current_class <- c() # Container for the ranks used for the control chart
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size2000[i] <- sum(ref_depths_pred <= RefinedRandomdepth_results_pred_size2000[i]) / length(ref_depths_pred)   # Normalized ranks 
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size3000[i] <- sum(ref_depths_pred <= RefinedRandomdepth_results_pred_size3000[i]) / length(ref_depths_pred)  # Normalized ranks 
...
for (i in 1:10000){
  ...
  ranks_testdata_pred_size4000[i] <- sum(ref_depths_pred <= RefinedRandomdepth_results_pred_size4000[i]) / length(ref_depths_pred)   # Normalized ranks 
...
PD_3_size2000 <- as.array(c(FAR_refsamples_size2000_mean, round(SR_pred_testdata_weighted_size2000, 2), round(CDR_outofcontrol_testdata_weighted_size2000, 2)))
PD_3_size3000 <- as.array(c(FAR_refsamples_size3000_mean, round(SR_pred_testdata_weighted_size3000, 2), round(CDR_outofcontrol_testdata_weighted_size3000, 2)))
PD_3_size4000 <- as.array(c(FAR_refsamples_size4000_mean, round(SR_pred_testdata_weighted_size4000, 2), round(CDR_outofcontrol_testdata_weighted_size4000, 2)))
